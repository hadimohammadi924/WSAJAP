    ...
    public class HomeController : Controller
    {
        ...
        public JsonResult ScheckCode(int personalcode, String Code)
        {
            ...
            if (temp.x2 == Code)
            {
                ...
                {
                    return Json(new { statue = "Success",idd=temp.suserid,post=temp.x3, id = temp.personalcode, name = temp.name,fname=temp.fname,phone=temp.phone,pic=temp.pic, }, JsonRequestBehavior.AllowGet);
                }
            }
            else
            {
